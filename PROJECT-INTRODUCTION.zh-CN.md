...
底层通过 OKX Agent Trade Kit（`okx` CLI）作为唯一执行内核，所有交易操作都经过 policy 审核和权限收口。用户看到的是对话，系统做到的是可控、可解释、可回放。
...
安装 TradeMesh skills 和安装任何 OpenClaw skill 一样简单。安装后，OpenClaw 自动发现新 skill 的能力，用户通过对话即可触发。不需要额外配置、不需要写编排脚本、不需要理解底层命令。
